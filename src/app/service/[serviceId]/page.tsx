...
export default async function ServiceTimePage({ params, searchParams }: Props) {
  ...
  if (!session) redirect(`/liff?next=/service/${serviceId}`)
  ...
  const [
    ...
    supabase.from('services').select('id, service_name, duration_minutes')
  ...
  // เช็คแต่ละ slot ว่า booked หรือยัง
  ...
  const slotInfo = slots.map(time => {
    ...
    const overlapping = (bookings ?? []).filter((b: any) => {
      ...
      const bEnd   = b.end_time
        ...
        : bStart + duration   // ถ้าไม่มี end_time เดาจาก duration
      return startMin < bEnd && endMin > bStart
    }).length

    const isPast = isToday && startMin < currentMin + 60  // ต้องล่วงหน้า 1 ชม.
    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      <div className="bg-white/80 backdrop-blur-lg border-b border-gray-100 sticky top-0 z-10">
        <div className="flex items-center gap-3 px-4 py-4">
          ...
          <div>
            <h1 className="font-bold text-gray-900 leading-tight">{service.service_name}</h1>
            <p className="text-xs text-gray-400 mt-0.5">{duration} นาที · {branch.name}</p>
          </div>
        </div>
      </div>
              ...
              {slotInfo.map(({ time, isPast, isFull }) => {
                ...
                return disabled ? (
                  ...
                  <SelectableLink key={time}
                    href={`/confirm?service=${serviceId}&date=${selectedDate}&time=${time}`}
                    prefetch={false}
